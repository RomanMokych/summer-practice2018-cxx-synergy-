...
LRESULT CALLBACK MouseEventProcServer(int nCode, WPARAM wParam, LPARAM lParam)
{
	...
	{
		if (InputHandler::Instance().hasConnection.load(std::memory_order_acquire))
		{
		...
		std::cout << InputHandler::Instance().isCurrentComputerDisabled.load(std::memory_order_acquire) << std::endl;
		return (InputHandler::Instance().isCurrentComputerDisabled.load(std::memory_order_acquire) ? 1 : CallNextHookEx(InputHandler::Instance().hKeyboardHook, nCode, wParam, lParam));
	}
}
